...
% Nach Spielebeendigung wird der "akutelle Spieler" noch angezeigt, obwohl das Spiel vorbei ist.
% Equals Methode für GameBoard
...
\begin{itemize}
	...
	\item Wie machen wir den Graphen im Backend?
	\begin{itemize}
		\item Graph wie bisher (Knoten und Kanten als eigene Klassen)
		\item Graph nur in AI mit Berechnung von anzuzeigenden Knoten immer ad hoc
		\item Graph lazy berechnen oder nicht?
	\end{itemize}
	\item Wollen wir die Isomorphismus-Klasse austauschbar machen, sodass der Graph in unterschiedlichen Settings unterschiedlich dargestellt werden kann?
	\item Wie ist die Player Klasse strukturiert / tut UserPlayer irgendetwas?
...
\newcommand{\ymax}{70}
	...
	\draw (5.5,0) node[below]{Sprint \(2\)};
	\draw (8.5,0) node[below]{Sprint \(3\)};
	%y-Achse Ticks
	...
	%Ziele
	\draw(4,7.5) node[cross, color=green]{};%Sprint 1
	\draw (7,34.5) node[cross, color=lightgray]{};%Sprint 2?
	\draw (10,64) node[cross, color=lightgray]{};%Sprint 3?
